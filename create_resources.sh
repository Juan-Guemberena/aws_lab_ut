...
rand=$RANDOM
USERNAME="${1}-${rand}"
echo $USERNAME >> usernames-with-numbers.txt
...
ROLE_A_ARN=`cat ${DATA_DIRECTORY}/role_A_output.json | jq -r .Role.Arn`
ROLE_B_ARN=`cat ${DATA_DIRECTORY}/role_B_output.json | jq -r .Role.Arn`

cat ${DATA_DIRECTORY}/role_A_output.json | jq -r .Role.Arn >> ${DATA_DIRECTORY}/role_a_arns.txt
cat ${DATA_DIRECTORY}/role_B_output.json | jq -r .Role.Arn >> ${DATA_DIRECTORY}/role_b_arns.txt

#Get the role and user ID for bucket 4
cat ${DATA_DIRECTORY}/role_A_output.json | jq -r .Role.RoleId >> ${DATA_DIRECTORY}/role_a_ids.txt
...
sed -e "s|\${RANDOM}|$RANDOM|" ${AWS_DIRECTORY}/bucket5_success.txt | tee ${DATA_DIRECTORY}/bucket5_ctf.txt >> ${DATA_DIRECTORY}/all_bucket5_ctfs.txt
...
sed -e "s|\${BUCKET_TWO}|awslab2-bucket-2|" ${AWS_DIRECTORY}/role_A_permissions_policy.json > ${DATA_DIRECTORY}/role_A_permissions_policy.json
sed -i "s|\${BUCKET_THREE}|awslab2-bucket-3|" ${DATA_DIRECTORY}/role_A_permissions_policy.json
sed -i "s|\${BUCKET_FOUR}|awslab-bucket-4|" ${DATA_DIRECTORY}/role_A_permissions_policy.json
...
sed -e "s|\${BUCKET_FOUR}|awslab-bucket-4|" ${AWS_DIRECTORY}/role_B_permissions_policy.json > ${DATA_DIRECTORY}/role_B_permissions_policy.json
